...
from lambda_connection_utils import (
    get_unrated_claims_from_input, get_context_from_lambdas, rate_claims_via_llm)
import json
...
def validate_event(event: dict) -> None:
    ...
    if any(key not in event for key in ["input", "input_type", "source_type"]):
        raise KeyError(
            "Event missing required keys input, input_type, source_type")
